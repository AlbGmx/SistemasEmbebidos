...
uint8_t isValidGuess(char playerGuess) { return isascii(playerGuess); }
...
void printResultToPlayerB(uint8_t resultado) {
	switch (resultado) {
		case NO_CHANGE:
			putStr(UART_CONSOLE, "Letra no encontrada!\n");
			break;
		case WORD_GUESSED:
			putStr(UART_CONSOLE, "Palabra adivinada!\n");
			break;
		case LETTER_FOUND:
			putStr(UART_CONSOLE, "Letra encontrada!\n");
			break;
		case NO_TRIES_LEFT:
			putStr(UART_CONSOLE, "Ya no quedan intentos\n Perdiste!\n");
			break;
		case LETTER_ALREADY_GUESSED:
			putStr(UART_CONSOLE, "Letra ya adivinada!\n");
		default:
			break;
	}
}
